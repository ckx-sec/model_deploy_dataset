...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < std::min(num, 5); ++i) {
        float x1 = output_data[i * stride + 0];
        float y1 = output_data[i * stride + 1];
        float x2 = output_data[i * stride + 2];
        float y2 = output_data[i * stride + 3];
        float score = output_data[i * stride + 4];
        std::cout << "Face " << i << ": x1=" << x1 << ", y1=" << y1 << ", x2=" << x2 << ", y2=" << y2 << ", score=" << score << std::endl;
    }
    return 0;
} 